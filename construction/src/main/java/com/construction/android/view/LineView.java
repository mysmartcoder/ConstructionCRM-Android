...
public class LineView extends View {
    ...
    private int topLineLength = MyUtils.dip2px(getContext(), 12);; // | | Ã¢â€ ï¿½this
                                                                   //-+-+-
	...
	//Ã¬Â ï¿½Ã¬â€žÂ Ã­â€˜Å“Ã¬â€¹Å“
    private Boolean drawDotLine = false;
    //Ã«ï¿½Â¼Ã¬ï¿½Â¸Ã¬Â»Â¬Ã«Å¸Â¬
...
//			getResources().getColor(R.color.color_equipment), getResources().getColor(R.color.color_desigm) };
        ...
        if(dataLists != null && !dataLists.isEmpty()){
        	...
        	for(int k = 0; k < dataLists.size(); k++){
        		...
        		for(int i=0;i<dataLists.get(k).size();i++){
                    ...
                    if(i>drawDotSize-1){
                    	//Ã«ï¿½â€žÃ­Å Â¸Ã«Â¦Â¬Ã¬Å Â¤Ã­Å Â¸Ã«Â¥Â¼ Ã¬Â¶â€�ÃªÂ°â‚¬Ã­â€¢Å“Ã«â€¹Â¤.
                        drawDotLists.get(k).add(new Dot(x, 0, x, y, dataLists.get(k).get(i),k));
                    }else{
                    	//Ã«ï¿½â€žÃ­Å Â¸Ã«Â¦Â¬Ã¬Å Â¤Ã­Å Â¸Ã¬â€”ï¿½ Ã­Æ’â‚¬ÃªÂ²Å¸Ã¬ï¿½â€ž Ã¬â€žÂ¤Ã¬Â â€¢Ã­â€¢Å“Ã«â€¹Â¤.
    ...
    //Ã«ï¿½â€žÃ­Å Â¸ÃªÂ·Â¸Ã«Â¦Â¬ÃªÂ¸Â°
...
//              canvas.drawText(String.valueOf(leftCount), 15, yCoordinateList.get(i), bottomTextPaint);
