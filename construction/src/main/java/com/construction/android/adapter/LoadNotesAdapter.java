...
import com.construction.android.adapter.LoadHistoryData.ViewHolder;
import com.construction.android.parser.NotesParser;

public class LoadNotesAdapter {
	MyFragmentActivity mActivity;
	ArrayList<NotesParser> mArrayList;
	ImageLoader mImageLoader;
	boolean fromActivit = false;

	public LoadNotesAdapter(MyFragmentActivity mActivity2, ArrayList<NotesParser> mNotesParsers, boolean isActivity) {
		...
		for (int position = 0; position < size; position++) {
			...
			if (mArrayList.get(position).getCheckSender().equalsIgnoreCase("")) {
				if (mArrayList.get(position).getAuthor().getGuid().equalsIgnoreCase(mActivity.getMyApplication().getUserID())
					...
					Calendar mCalendar = Calendar.getInstance();
					mCalendar.setTimeInMillis(Long.parseLong(mArrayList.get(position).getTimestamp()));
					holder.mTextViewDate.setText(mActivity.getCommonMethod().relatvieTimveSpan(
							Long.parseLong(mArrayList.get(position).getTimestamp())));
	...
	// if (convertView == null) {
	// holder = new ViewHolder();
	// convertView = mActivity.getLayoutInflater().inflate(R.layout.row_notes,
	// null);
	//
	// holder.mTextViewTitle = (TextView)
	// convertView.findViewById(R.id.row_notes_textview_title);
	// holder.mTextViewDate = (TextView)
	// convertView.findViewById(R.id.row_notes_textview_date);
	// holder.mTextViewSenderMessage = (TextView)
	// convertView.findViewById(R.id.row_notes_textview_sender_title);
	// holder.mTextViewSenderDate = (TextView)
	// convertView.findViewById(R.id.row_notes_textview_sender_date);
	// holder.mImageViewSender = (ImageView)
	// convertView.findViewById(R.id.row_notes_imageview_sender);
	//
	// holder.mRelativeLayoutReciever = (RelativeLayout)
	// convertView.findViewById(R.id.row_notes_relative_reciver_message);
	// holder.mRelativeLayoutSender = (RelativeLayout)
	// convertView.findViewById(R.id.row_notes_relative_sender_message);
	//
	...
	// if (mArrayList.get(position).getCheckSender().equalsIgnoreCase("")) {
	// if
	// (mArrayList.get(position).getAuthor().getGuid().equalsIgnoreCase(mActivity.getMyApplication().getUserID())
	// || mArrayList.get(position).getAuthor().getGuid().equalsIgnoreCase("")) {
	//
	// holder.mTextViewSenderMessage.setText(mArrayList.get(position).getMessage());
	// Calendar mCalendar = Calendar.getInstance();
	...
	// mArrayList.get(position).getSenderImagePath(), holder.mImageViewSender,
	// mActivity.getMyApplication().profileOption);
	// }
	// holder.mRelativeLayoutSender.setVisibility(View.VISIBLE);
	// holder.mRelativeLayoutReciever.setVisibility(View.GONE);
